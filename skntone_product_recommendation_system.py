...
"""
# SKNTONE Product Recommender Streamlit App (with 3 display options)
...
display_option = st.selectbox("How would you like to see your recommendations?", ["Simple List", "Emoji Enhanced", "Side-by-Side Columns"])

if user_input:
    recommendations = multi_concern_recommender(user_input)
    st.subheader("Recommended Products for You:")

    if display_option == "Simple List":
    ...
    elif display_option == "Emoji Enhanced":
        emoji_map = {
            'oil': '💛',
            'wash': '🫧',
            'cream': '👐',
        ...
        for _, row in recommendations.iterrows():
            ...
            st.markdown(f"""
            ### {emoji} {row['product']}
            - **Concern:** {row['matched_concern']}
            - **Sentiment Score:** `{round(row['sentiment'], 2)}`
            """)

    elif display_option == "Side-by-Side Columns":
